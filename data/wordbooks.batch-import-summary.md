...
- 词表识别已覆盖 `Words and expressions`、`Word List`、`Vocabulary (I)/(II)`、`单词表`、`词汇表` 等版式。
...
本次为客户授权官方电子教材书后词表的自动 OCR 批量导入，已保留 `source_url` 和 `authorization_id` 供回溯。校验报告显示空书为 0；以下 4 册词量低于 200，建议正式上线前重点人工抽检：
